...
exports['Should correctly connect to proxy and use readPreference secondary'] = {
  ...
  test: function(configuration, test) {
    ...
    // Url for connection to proxy
    var url = 'mongodb://localhost:51000/test?readPreference=secondary';
    ...
    MongoClient.connect(url, function(err, db) {
      ...
      // Perform an inserts
      db.collection('t3').insert([{a:1}, {b:1}, {c:1}, {d:1}], function(err, r) {
        test.equal(null, err);

        db.collection('t3').find({}).batchSize(2).toArray(function(err, docs) {
          test.equal(null, err);
...
exports['Concurrent cursors against secondary'] = {
  ...
  test: function(configuration, test) {
    ...
    MongoClient.connect(url, function(err, db) {
      ...
      // Perform an inserts
      db.collection('t4').insert([{a:1}, {b:1}, {c:1}, {d:1}], {w:'majority'}, function(err, r) {
        test.equal(null, err);
        ...
        for(var i = 0; i < total; i++) {
          db.collection('t4').find({}).batchSize(2).toArray(function(err, docs) {
            test.equal(null, err);
            numberLeft = numberLeft - 1;
